...
void child_only_signal(int signum)
{
  if (getpid() == parent_pid)
  {}
  else if (signum == 2)
  {
    printf("%s Child %d is terminating\n\n", debug_note, getpid());
    exit(0);
  }
  else
  {
    printf("%s Child %d is stopped\n\n", debug_note, getpid());
    kill(getpid(), SIGSTOP);
  }
}
...
void execute(char **argv, char *raw_line_input)
{
     ...
     signal(SIGINT, child_only_signal);
     signal(SIGTSTP, child_only_signal);
	 ...
	 printf("%s Child %d is executing <%s>\n\n", debug_note, getpid(), raw_line_input);
	 if (execvp(*(argv), argv) == 0)
	     printf("%s Child %d exited normally\n\n", debug_note, getpid());
	 else
	     printf("%s execvp() failed to execute the command\n\n", debug_note); 
	 exit (0);
       } else
       ...
       { /* for the parent:      */
         ...
         // kinds of waits...
         printf("%s Parent %d is waiting...\n\n", debug_note, getpid());
         wpid = waitpid(pid, &status, 0);
	 printf("\n%s Parent %d is resuming \n\n", debug_note, getpid());
...
int main(int argc, char **argv, char **envp)
{
     ...
     // set them here?
     signal(SIGINT, ignore_signal); // this will block ctrl+c
     signal(SIGTSTP, ignore_signal); // this will stop ctrl+z
     ...
     while (1)
       {  // Maybe once per processing loop you could try to check if there's
          ...
          if ((*line != '\0') && (*line > 31)) // If something was actually typed, then do something...
            {
              ...
              if (strcmp(line_argv[0], "printenv")  == 0) { char **temp_envp;
							    temp_envp = envp;
	      ...
	      if (strcmp(line_argv[0], "pwd")       == 0) { char pwd[256];
	      ...
	      if (strcmp(line_argv[0], "cd") 	    == 0) { if (line_argv[1] == NULL)
							    {
							    ...
							    if (chdir(line_argv[1]) != 0)
   							    {
							      printf("No such file or directory as %s\n\n", line_argv[1]);
							    } else
