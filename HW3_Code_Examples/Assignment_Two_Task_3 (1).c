...
static int send_num_msgs = 10;              // A NEW GLOBAL STATIC VARIABLE TO DETERMINE
                                            // HOW MANY TIMES THE MESSAGE WILL BE SENT
                                            // TO THE USER
...
static ssize_t device_read(struct file *flip, char *buffer, size_t len, loff_t *offset)
  { int bytes_read = 0;
     ...
     // THE SEND_NUM_MSGS VARIABLE DECREMENTS EVERY TIME *MSG_PTR REACHES THE END OF THE
     // MESSAGE, AND WILL BE USED AS A CONDITION IN THE WHILE LOOP THAT OUTPUTS THE
     // MESSAGE
     ...
     // THE LOOP CAN NOW ONLY SEND THE MESSAGE 10 TIMES BECAUSE OF THE SEND_NUM_MSGS > 0
     // CONDITION. THE LOOP WILL STOP WHEN THE MSG_PTR IN PUT_USER REACHES THE END OF THE 
     // MESSAGE AND CONSEQUENTLY DECREMENTS THE VARIABLE WHEN IT RESETS TO THE BEGGINING 
     // IN THE IF STATEMENT ABOVE. THIS MEANS THE ENTIRE MESSAGE GETS SENT 10 TIMES.

        while (len && *msg_ptr && send_num_msgs > 0)
...
static int device_open(struct inode *inode, struct file *file)
   { /* If device is open, return busy */
     ...
     device_open_count++;
     send_num_msgs = 10; // ENSURES THE VALUE IS INITIALLY SET TO 10
...
static int device_release(struct inode *inode, struct file *file)
  { device_open_count--;
    send_num_msgs = 10; //ENSURES THE VALUE IS RESET TO 10 WHEN CLOSED/RELEASED
